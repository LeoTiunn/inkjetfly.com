<?php
/*
  $Id: paypal_wpp_ipn.php,v 1.0.0.0 2007/11/13 13:41:11 datazen Exp $
...
*/
  ...
  include(DIR_WS_MODULES . 'payment/paypal_wpp/paypal_wpp_base.php');

  if ( MODULE_PAYMENT_PAYPAL_WPP_DEBUGGING == 'True' ) {
    $message = print_r($_POST, true);
    $log_file = DIR_FS_CATALOG . 'debug/paypal_wpp_debug.txt';
    $fp = @fopen($log_file, 'a');
    @fwrite($fp, 'Log Time: ' . date('Y-m-d H:i:s') . ' by: ' . $_SERVER['PHP_SELF'] . "\n");
    @fwrite($fp, 'Paypal IPN: ' . $message . "\n\n");
    @fclose($fp);
  }
  
  $payment_date = paypal_wpp_base::get_payment_date($_POST['payment_date']);
  $sql_data = array('mc_gross' => $_POST['mc_gross'],
                    ...
                    'payment_date' => $payment_date['date'] . ' ' . $payment_date['time'],
  ...
  switch ($_POST['payment_status']) {
    case 'Refunded':
      ...
      tep_db_query("update " . TABLE_ORDERS . " set orders_status = '" . MODULE_PAYMENT_PAYPAL_WPP_REFUND_ORDER_STATUS_ID . "', last_modified = 'now()' where orders_id = '" . $order['orders_id'] . "'");
      $sql_data = array('orders_id' => $order['orders_id'],
                        'orders_status_id' => MODULE_PAYMENT_PAYPAL_WPP_REFUND_ORDER_STATUS_ID,
                        'date_added' => 'now()',
