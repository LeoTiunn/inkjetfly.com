    ...
    if(!$table_srp_exists || ($last_update_table_specials > $last_update_table_srp)) {
      if ($table_srp_exists) {
        ...
        if (tep_db_query($query1)) {
          $query2 = "insert into " . TABLE_SPECIALS_RETAIL_PRICES . " select products_id, specials_new_products_price, status from " . TABLE_SPECIALS;
          $result =  tep_db_query($query2);
        }
      } else { // table specials_retail_prices does not exist
         $query1 = "create table " . TABLE_SPECIALS_RETAIL_PRICES . " (products_id int NOT NULL default '0', specials_new_products_price decimal(15,4) NOT NULL default '0.0000', status tinyint, primary key (products_id) )" ;
         $query2 = "insert into " . TABLE_SPECIALS_RETAIL_PRICES . " select products_id, specials_new_products_price, status from " . TABLE_SPECIALS;
