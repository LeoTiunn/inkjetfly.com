...
class creAttributes {
 /* private $products_id = '';  // identifies which product information is held for
  private $tax_class = '';    // the products tax class needed for option pricing
  private $load_id = '';      // in case of a subproduct, identifies product information loaded
  ...
  private $HTML_tags = array();  // hold the option/value as HTML input tags - not cached in the session*/
   var $products_id, $tax_class, $load_id, $options= array(), $values= array(),$HTML_tag= array();
  // class constructor
 function __construct() { 
   ...
   function load($id = '') {
    ...
    if ($id == $this->products_id) return true;
    ...
    if ($product_to_load == $this->load_id) return true; // the information is already loaded
