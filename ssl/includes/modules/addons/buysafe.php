  ...
  class buysafe {
    ...
    function call_api($method, $params = array(), $order_id = '') {
      ...
      case 'SetShoppingCartCheckout':
        ...
        //$soapclient = new soapclient(MODULE_ADDONS_BUYSAFE_API_URL, false, false, false, false, false, 5, 5);
      ...
      case 'SetShoppingCartCancelOrder':
        ...
        $body = '<SetShoppingCartCancelOrderRQ xmlns="http://ws.buysafe.com">
                   ...
                   <SessionId>' . $this->str_encode($params['buysafe_session_id']) . '</SessionId>
    ...
    function keys() {
      return array('MODULE_ADDONS_BUYSAFE_STATUS', 'MODULE_ADDONS_BUYSAFE_SEAL_TYPE', 'MODULE_ADDONS_BUYSAFE_SEAL_AUTHENTICATION_DATA', 'MODULE_ADDONS_BUYSAFE_STORE_AUTHENTICATION_DATA', 'MODULE_ADDONS_BUYSAFE_DEBUG', /*'MODULE_ADDONS_BUYSAFE_PLATFORM', 'MODULE_ADDONS_BUYSAFE_API_URL', 'MODULE_ADDONS_BUYSAFE_ROLLOVER_URL', 'MODULE_ADDONS_BUYSAFE_STOP_API_CALLS_TIME'*/);
    }

    function hidden_keys() {
      return array('MODULE_ADDONS_BUYSAFE_CART_PREFIX', 'MODULE_ADDONS_BUYSAFE_PLATFORM', 'MODULE_ADDONS_BUYSAFE_TOK', 'MODULE_ADDONS_BUYSAFE_API_URL', 'MODULE_ADDONS_BUYSAFE_ROLLOVER_URL', 'MODULE_ADDONS_BUYSAFE_STOP_API_CALLS_TIME');
    }
    ...
    function install() {
      ...
      // hide below configs from editing.
      //     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Checkout API URL', 'MODULE_ADDONS_BUYSAFE_API_URL', 'https://api.buysafe.com/buysafews/checkoutapi.dll', 'The post URL for the Checkout API.', '0', '8', now())");
      //     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Rollover JS URL', 'MODULE_ADDONS_BUYSAFE_ROLLOVER_URL', 'https://seal.buysafe.com/private/rollover/rollover.js', 'The code to process the rollover is provided by buySAFE.', '0', '9', now())");
      //     tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Stop API Calls Time', 'MODULE_ADDONS_BUYSAFE_STOP_API_CALLS_TIME', '', 'Stop API calls to this datetime.', '0', '10', now())");
      ...
      tep_db_query("INSERT INTO infobox_configuration VALUES ('" . $template_id . "', '', 'buysafe.php', 'BOX_HEADING_BUYSAFE', 'yes', 'left', 1, now(), now(), 'buySAFE', 'infobox', '#FFFFFF')"); 
      // get infobox_id
      ...
      tep_db_query("CREATE TABLE IF NOT EXISTS `buysafe` (
                  ...
                  `buysafe_session_id` VARCHAR( 64 ) NOT NULL,
                   PRIMARY KEY  (buysafe_id, orders_id)) TYPE=MyISAM;");
                   
      // inject database variable into configuration table
      tep_db_query("REPLACE INTO " . TABLE_CONFIGURATION . " VALUES ('', '','TABLE_BUYSAFE', 'buysafe', '', '', '', now(), now(), NULL, NULL)");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " values ('','Checkout API URL', 'MODULE_ADDONS_BUYSAFE_API_URL', 'https://api.buysafe.com/buysafews/checkoutapi.dll', 'The post URL for the Checkout API.', '', '', now(),now(),NULL,NULL)");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " values ('','Rollover JS URL', 'MODULE_ADDONS_BUYSAFE_ROLLOVER_URL', 'https://seal.buysafe.com/private/rollover/rollover.js', 'The code to process the rollover is provided by buySAFE.', '', '', now(),now(),NULL,NULL)");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " values ('','buySafe Cart Prefix', 'MODULE_ADDONS_BUYSAFE_CART_PREFIX', 'CRELoade', 'buySafe Cart Prefix.', '', '', now(),now(),NULL,NULL)");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " values ('','buySafe Token', 'MODULE_ADDONS_BUYSAFE_TOK', 'C00ACE2A-3097-44A1-933A-B81AEDC40ACB', 'buySafe Token.', '', '', now(),now(),NULL,NULL)");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " values ('','buySAFE Platform', 'MODULE_ADDONS_BUYSAFE_PLATFORM', 'CRELoaded', 'The platform as provided by buySAFE.', '', '', now(),now(),NULL,NULL)");
    ...
    function get_cart_items($order_id = '') {
      ...
      if ($order) {
        ...
        foreach ($order->products as $product) {
          ...
          $items_string .= '<ShoppingCartItem>
                              ...
                              <MarketplaceItemCode>' . $this->str_encode((tep_not_null($product['model']) && $check_unique['total'] == 1) ? $product['model'] : $product['name']) . '-' . $i . '</MarketplaceItemCode>
      ...
      if ($cart->count_contents() > 0) {
        ...
        for ($i=0, $n=sizeof($products); $i<$n; $i++) {
            ...
            while (list($option, $value) = each($products[$i]['attributes'])) {
              if ( !is_array($value) ) {
                ...
                $attributes_query = tep_db_query("select op.options_id, ot.products_options_name, o.options_type, op.options_values_price, op.price_prefix
                                                  ...
                                                  where op.products_id = '" . tep_get_prid($products[$i]['id']) . "'
                                                    and op.options_id = '" . $option . "'
                                                    and o.not products_options_id = '" . $option . "'
                                                    and ot.products_options_text_id = '" . $option . "'
                                                    and ot.language_id = '" . (int)$languages_id . "'
                                                 ");
                $attributes = tep_db_fetch_array($attributes_query);
          ...
          $items_string .= '<ShoppingCartItem>
                              <UserToken>' . $this->str_encode(MODULE_ADDONS_BUYSAFE_STORE_AUTHENTICATION_DATA) . '</UserToken>
                              <MarketplaceItemCode>' . $this->str_encode((tep_not_null($products[$i]['model']) && $check_unique['total'] == 1) ? $products[$i]['model'] : $products[$i]['name']) . '-' . ($i + 1) . '</MarketplaceItemCode>
