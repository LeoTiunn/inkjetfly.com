...
// class constructor
    function fedex1() {
      ...
      if (defined('MODULE_SHIPPING_FEDEX1_SORT_ORDER')) {
        $this->sort_order = (int)MODULE_SHIPPING_FEDEX1_SORT_ORDER;
      } else {
        $this->sort_order = '';
      }

      $this->icon = DIR_WS_ICONS . 'shipping_fedex.gif';
      if (defined('MODULE_SHIPPING_FEDEX1_TAX_CLASS')) {
        $this->tax_class = (int)MODULE_SHIPPING_FEDEX1_TAX_CLASS;
      } else {
        $this->tax_class = 0;
      }
      ...
      if (defineD('MODULE_SHIPPING_FEDEX1_METER')) {
...
/*
      ...
      $this->international_types = array(
             '01' => MODULE_SHIPPING_FEDEX1_INTERNATIONAL_TYPES_01,
             '03' => MODULE_SHIPPING_FEDEX1_INTERNATIONAL_TYPES_03,
             '06' => MODULE_SHIPPING_FEDEX1_INTERNATIONAL_TYPES_06,
             '90' => MODULE_SHIPPING_FEDEX1_INTERNATIONAL_TYPES_90,
             '92' => MODULE_SHIPPING_FEDEX1_INTERNATIONAL_TYPES_92
             );
...
// class methods
    function quote($method = '') {
      ...
      $this->quotes = array('module' => $this->title,
                              'error' => MODULE_SHIPPING_FEDEX1_ERROR_1);
      }
    ...
    function _getMeter() {
      ...
      //if (MODULE_SHIPPING_FEDEX1_DEBUG == 'True') $this->write_log("Data sent to Fedex for Meter: " . $data);
      if (MODULE_SHIPPING_FEDEX1_DEBUG == 'True') $this->write_log(MODULE_SHIPPING_FEDEX1_MESSAGE_1 . $data);
      ...
      if (MODULE_SHIPPING_FEDEX1_DEBUG == 'True') $this->write_log(MODULE_SHIPPING_FEDEX1_MESSAGE_2 . $fedexData);
      ...
      if ($meterStart === FALSE) {
        if (strlen($fedexData) == 0) {
          ...
          $this->error_message = MODULE_SHIPPING_FEDEX1_MESSAGE_3;
        } else {
         ...
         // $this->error_message = 'No meter number was obtained, check configuration. Error ' . $fedexData['2'] . ' : ' . $fedexData['3'];
          $this->error_message = MODULE_SHIPPING_FEDEX1_MESSAGE_4 . $fedexData['2'] . ' : ' . $fedexData['3'];
        }
        return false;
      }
    ...
    function _getQuote() {
      ...
      if (MODULE_SHIPPING_FEDEX1_ACCOUNT == "NONE" || strlen(MODULE_SHIPPING_FEDEX1_ACCOUNT) == 0) {
       // return array('error' => 'You forgot to set up your Fedex account number, this can be set up in Admin -> Modules -> Shipping');
        return array('error' => MODULE_SHIPPING_FEDEX1_MESSAGE_5);
      }
      if (MODULE_SHIPPING_FEDEX1_ADDRESS_1 == "NONE" || strlen(MODULE_SHIPPING_FEDEX1_ADDRESS_1) == 0) {
       // return array('error' => 'You forgot to set up your ship from street address line 1, this can be set up in Admin -> Modules -> Shipping');
        return array('error' => MODULE_SHIPPING_FEDEX1_MESSAGE_6);
      }
      if (MODULE_SHIPPING_FEDEX1_CITY == "NONE" || strlen(MODULE_SHIPPING_FEDEX1_CITY) == 0) {
       // return array('error' => 'You forgot to set up your ship from City, this can be set up in Admin -> Modules -> Shipping');
        return array('error' => MODULE_SHIPPING_FEDEX1_MESSAGE_7);
      }
      ...
      //  return array('error' => 'You forgot to set up your ship from postal code, this can be set up in Admin -> Modules -> Shipping');
        return array('error' => MODULE_SHIPPING_FEDEX1_MESSAGE_8);
      }
      if (MODULE_SHIPPING_FEDEX1_PHONE == "NONE" || strlen(MODULE_SHIPPING_FEDEX1_PHONE) == 0) {
        ...
        return array('error' => MODULE_SHIPPING_FEDEX1_MESSAGE_9);
      }
    ...
    //  if (MODULE_SHIPPING_FEDEX1_DEBUG == 'True') $this->write_log("Data sent to Fedex for Rating: " . $data);
      if (MODULE_SHIPPING_FEDEX1_DEBUG == 'True') $this->write_log(MODULE_SHIPPING_FEDEX1_MESSAGE_10 . $data);
      $fedexData = $this->_AccessFedex($data);
      //if (MODULE_SHIPPING_FEDEX1_DEBUG == 'True') $this->write_log("Data returned from Fedex for Rating: " . $fedexData);
      if (MODULE_SHIPPING_FEDEX1_DEBUG == 'True') $this->write_log(MODULE_SHIPPING_FEDEX1_MESSAGE_11 . $fedexData);
      if (strlen($fedexData) == 0) {
     //   $this->error_message = 'No data returned from Fedex, perhaps the Fedex site is down';
        $this->error_message = MODULE_SHIPPING_FEDEX1_MESSAGE_12;
      ...
      if (is_array($rates)) {
        ...
        $this->error_message = MODULE_SHIPPING_FEDEX1_MESSAGE_13 . $fedexData['2'] . ' : ' . $fedexData['3'];
        return array('error' => $this->error_message);
      }
    ...
    function write_log($message, $log_file = 'debug/fedex_debug.txt') {
      $fp = @fopen($log_file, 'a');
