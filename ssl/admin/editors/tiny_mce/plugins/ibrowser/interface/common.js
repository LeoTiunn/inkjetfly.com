...
// ================================================ 
  ...
  function iBrowser_open() {      
    ...
    // open iBrowser dialog 
        // Kiran modified below code to pass osCAdminID for security
    if (ib.isMSIE) { // IE
      var rArgs = showModalDialog(ib.baseURL + '?osCAdminID=' + AdminID, wArgs, 'dialogHeight:500px; dialogWidth:580px; scrollbars: no; menubar: no; toolbar: no; resizable: no; status: no;');  
      if (rArgs) { // returning from iBrowser (IE) and calling callback function        
        iBrowser_callback('','',rArgs);
      }
    } else if (ib.isGecko) { // Gecko 
      var wnd = window.open(ib.baseURL + '?editor=' + ib.editor + '&callback=iBrowser_callback&osCAdminID=' + AdminID, 'ibrowser', 'status=no, modal=yes, width=625, height=530');
