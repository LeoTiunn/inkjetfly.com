<?php
    ...
    $cfg['umax']           = 3;                                                        // max. number of image files to be uploaded; default: 1; value > 1
    ...
    //get small image width from config
    $cfg['thumbs'][1] = array (                                                                        
            'size'       => ((defined('SMALL_IMAGE_WIDTH') && SMALL_IMAGE_WIDTH != '') ? SMALL_IMAGE_WIDTH : '100'),                                                
            'ext'        => '*',
            'crop'        => false,
    );
    //get medium image width from config
    $cfg['thumbs'][2] = array (                                                                        
            'size'       => ((defined('MEDIUM_IMAGE_WIDTH') && MEDIUM_IMAGE_WIDTH != '') ? SMALL_IMAGE_WIDTH : '150'),                                                
            'ext'        => '*',
            'crop'        => false,
    );
  /*    //get extra image width from config
    if(ULT_THUMB_IMAGE_WIDTH != ''){
        $cre_ult_size = ULT_THUMB_IMAGE_WIDTH;
    } else {
        $cre_ult_size = '150';
    }
    $cfg['thumbs'][3] = array (                                                                        
    ...
    $cfg['root_dir']    = substr_replace(DIR_FS_CATALOG,'',-1);//'/home/wa4u/www/cre63/b2b';//((@$_SERVER['DOCUMENT_ROOT'] && file_exists(@$_SERVER['DOCUMENT_ROOT'] . $_SERVER['PHP_SELF'])) ? $_SERVER['DOCUMENT_ROOT'] : str_replace(dirname(@$_SERVER['PHP_SELF']), '', str_replace('\\', '/', realpath('.'))));
    $cfg['base_url']    = 'http://' . $_SERVER['SERVER_NAME'] . substr_replace(DIR_WS_HTTP_CATALOG,'',-1);                             // base url; e.g. 'http://localhost/'    
    
    $cfg['main_dir']    = ereg_replace("//", "/", dirname($_SERVER['PHP_SELF']));// iBrowser main dir; e.g. '/home/domain/public_html/ibrowser/'    
    ...
    $cfg['temp']        = DIR_FS_CATALOG . 'tmp'; // temp dir; e.g. 'D:/www/temp'    
?>