  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'setflag':
        ...
        tep_db_perform(TABLE_SALEMAKER_SALES, $salemaker_data_array, 'update', "sale_id = '" . tep_db_input(tep_db_prepare_input($_GET['sID'])) . "'");
...
// Create a string of all affected (sub-)categories
        ...
        $salemaker_sales_data_array = array('sale_name' => tep_db_prepare_input($_POST['name']),
                                            ...
                                            'sale_date_start' => tep_db_prepare_input($_POST['start']),
                                            'sale_date_end' => tep_db_prepare_input($_POST['end']) );
...
<?php
  if ( ($action == 'new') || ($action == 'edit') ) {
    ...
    if ( ($action == 'edit') && ($_GET['sID']) ) {
      ...
      $sInfo->sale_status = '';
      $sInfo->sale_name = '';
      $sInfo->sale_deduction_value = '';
      $sInfo->sale_deduction_type = '';
      $sInfo->sale_pricerange_from = '';
      $sInfo->sale_pricerange_to = '';
      $sInfo->sale_specials_condition = '';
      $sInfo->sale_categories_selected = '';
      $sInfo->sale_categories_all = '';
...
?>
<script language="javascript">
var StartDate = new ctlSpiffyCalendarBox("StartDate", "sale_form", "start", "btnDate1","<?php echo (($sInfo->sale_date_start == '0000-00-00') ? '' : $sInfo->sale_date_start); ?>",scBTNMODE_CUSTOMBLUE);
var EndDate = new ctlSpiffyCalendarBox("EndDate", "sale_form", "end", "btnDate2","<?php echo (($sInfo->sale_date_end == '0000-00-00') ? '' : $sInfo->sale_date_end); ?>",scBTNMODE_CUSTOMBLUE);
</script>
      <tr><form name="sale_form" <?php echo 'action="' . tep_href_link(FILENAME_SALEMAKER, tep_get_all_get_params(array('action', 'info', 'sID')) . 'action=' . $form_action, 'NONSSL') . '"'; ?> method="post"><?php if ($form_action == 'update') echo tep_draw_hidden_field('sID', $_GET['sID']); ?>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="main" align="right" valign="top"><br><?php echo (($form_action == 'insert') ? tep_image_submit('button_insert.gif', IMAGE_INSERT) : tep_image_submit('button_update.gif', IMAGE_UPDATE)). '&nbsp;&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_SALEMAKER, 'page=' . (isset($_GET['page']) ? $_GET['page'] : 0) . '&sID=' . (isset($_GET['sID']) ? $_GET['sID'] : 0)) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL); ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
<?php
    ...
    echo "      <tr>\n";
    echo '        <td valign="bottom">' . tep_draw_separator('pixel_trans.gif', '4', '1') . tep_image(DIR_WS_IMAGES . 'arrow_right_blue.png') . "</td>\n";
...
<?php
    ...
    while ($salemaker_sales = tep_db_fetch_array($salemaker_sales_query)) {
      ...
      if (isset($sInfo) && is_object($sInfo) && ($salemaker_sales['sale_id'] == $sInfo->sale_id)) {
        echo '                  <tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_SALEMAKER, 'page=' . (isset($_GET['page']) ? (int)$_GET['page'] : 0) . '&sID=' . $salemaker_sales['sale_id'] . '&action=edit') . '\'">' . "\n";
      } else {
        echo '                  <tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_SALEMAKER, 'page=' . (isset($_GET['page']) ? (int)$_GET['page'] : 0) . '&sID=' . $salemaker_sales['sale_id']) . '\'">' . "\n";
      }
?>
...
<?php
      if ($salemaker_sales['sale_status'] == '1') {
        echo tep_image(DIR_WS_IMAGES . 'accept.png', IMAGE_ICON_STATUS_GREEN, 16, 16) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_SALEMAKER, 'action=setflag&flag=0&sID=' . $salemaker_sales['sale_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'cancel-off.png', IMAGE_ICON_STATUS_RED_LIGHT, 16, 16) . '</a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_SALEMAKER, 'action=setflag&flag=1&sID=' . $salemaker_sales['sale_id'], 'NONSSL') . '">' . tep_image(DIR_WS_IMAGES . 'accept-off.png', IMAGE_ICON_STATUS_GREEN_LIGHT, 16, 16) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'cancel.png', IMAGE_ICON_STATUS_RED, 16, 16);
      }
?></td>
                <td class="dataTableContent" align="right"><?php if ( (isset($sInfo) && is_object($sInfo)) && ($salemaker_sales['sale_id'] == $sInfo->sale_id) ) { echo tep_image(DIR_WS_IMAGES . 'arrow_right_blue.png', ''); } else { echo '<a href="' . tep_href_link(FILENAME_SALEMAKER, 'page=' . $_GET['page'] . '&sID=' . $salemaker_sales['sale_id']) . '">' . tep_image(DIR_WS_IMAGES . 'information.png', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
      </tr>
<?php
    }
?>
              ...
              <tr>
                <td colspan="7"><table border="0" width="100%" cellpadding="0"cellspacing="0">
                  <tr>
  ...
  switch ($action) {
    ...
    default:
      if (isset($sInfo) && is_object($sInfo)) {
        ...
        $contents[] = array('align' => 'center', 'text' => '<br><a href="' . tep_href_link(FILENAME_SALEMAKER, 'page=' . $_GET['page'] . '&sID=' . $sInfo->sale_id . '&action=edit') . '">' . tep_image_button('button_page_edit.png', IMAGE_EDIT) . '</a><a href="' . tep_href_link(FILENAME_SALEMAKER, 'page=' . $_GET['page'] . '&sID=' . $sInfo->sale_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a><br>' . tep_draw_separator('pixel_trans.gif', '1', '20') . '<a href="' . tep_href_link(FILENAME_SALEMAKER, 'page=' . $_GET['page'] . '&sID=' . $sInfo->sale_id . '&action=copy') . '">' . tep_image_button('button_copy_to.gif', IMAGE_COPY_TO) . '</a>');
