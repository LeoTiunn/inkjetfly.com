...
-->
      <tr>
        ...
        if ($sel_month<>0)   {
        echo "<a href='" . tep_href_link(basename($_SERVER['PHP_SELF']),'selected_box=reports') . "";
        ...
        echo get_href_url().$_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING'] . "&print=yes";
        ?>" target="print" title="<?php echo TEXT_BUTTON_REPORT_PRINT_DESC . "\">" . TEXT_BUTTON_REPORT_PRINT; ?></a>
        </td>
        <td class="smallText"><a href='<?php echo get_href_url().$_SERVER['PHP_SELF'] . "?" . ereg_replace('&invert=yes','',$_SERVER['QUERY_STRING']);
        ...
        echo tep_href_link(basename($_SERVER['PHP_SELF'])) . "?&help=yes"; ?>','help',config='height=400,width=600,scrollbars=1, resizable=1')" title="<?php echo TEXT_BUTTON_REPORT_HELP_DESC . "\">" . TEXT_BUTTON_REPORT_HELP; ?></a>
        </td>
        </tr></table>
        </td>
      </tr>
...
if ($sel_month == 0 && !$print) {
  echo "<a href='" .get_href_url(). $_SERVER['PHP_SELF'] . "?" . $_SERVER['QUERY_STRING'] . "&month=" . $sales['i_month'] . "&year=" . $sales['row_year'] . "' title='" . TEXT_BUTTON_REPORT_GET_DETAIL . "'>";
  }
...
<?php 
  ...
  if (!$print && ($net_sales_this_row['tax']>0)) {
    ...
    echo "<a href=\"#\" onClick=\"window.open('" . tep_href_link(basename($_SERVER['PHP_SELF']),$tmp_link);    
          ...
          <table class="data-table-foot" border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>            
              <td align="right" class="button-container">
                <form action="<?php echo get_href_url().$_SERVER['PHP_SELF'].(isset($_GET['osCAdminID'])?'?osCAdminID='.$_GET['osCAdminID']:''); ?>" method=post><input type='hidden' name='csv' value='<?php echo $csv_accum; ?>'><input type='hidden' name='saveas' value='sales_report_<?php
