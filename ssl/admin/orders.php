      ...
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td width="100%" class="main" align="right"><table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr>
                <td class="main" align="right">
                <?php  
                if  (strstr(strtolower($order->info['payment_method']), 'paypal') && (isset($_GET['referer'])) && ($_GET['referer'] == 'ipn')) { 
                  echo '<a href="' . tep_href_link(FILENAME_PAYPAL, tep_get_all_get_params(array('action','oID','referer')), 'SSL') . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>';
                } else { 
                  echo '<a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('action','referer')), 'SSL') . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>';
                }  
 ...
 /*       if (isset($_SESSION['is_std']) && $_SESSION['is_std'] === true) {
    ...
    //    }
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td valign="top"><?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE); ?> <?php echo '<a href="' . tep_href_link(FILENAME_ORDERS_USPS_SHIP, 'oID=' . $HTTP_GET_VARS['oID']) . '" TARGET="_blank">' .  tep_image('includes/languages/english/images/buttons/button_labelshipping.gif', IMAGE_ORDERS_USPS_SHIP) . '</a> '; ?></td>
          </tr>
          <?php
          // RCI start
      ...
      <tr>
        ...
        if (strstr(strtolower($order->info['payment_method']), 'paypal') && isset($_GET['referer']) && $_GET['referer'] == 'ipn'){
          ?>
          <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_PAYPAL, tep_get_all_get_params(array('action')), 'SSL') . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a><a href="javascript:popupWindow(\'' . (HTTPS_SERVER . DIR_WS_HTTPS_ADMIN . FILENAME_ORDERS_INVOICE) . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a><a href="javascript:popupWindow(\'' . (HTTPS_SERVER . DIR_WS_HTTPS_ADMIN . FILENAME_ORDERS_PACKINGSLIP) . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a>' . $buttons_bottom; ?></td>
          <?php
        } else { //not paypal
          ?>
          <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('action')), 'SSL') . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a><a href="javascript:popupWindow(\'' . (HTTPS_SERVER . DIR_WS_HTTPS_ADMIN . FILENAME_ORDERS_INVOICE) . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_invoice.gif', IMAGE_ORDERS_INVOICE) . '</a><a href="javascript:popupWindow(\'' . (HTTPS_SERVER . DIR_WS_HTTPS_ADMIN . FILENAME_ORDERS_PACKINGSLIP) . '?' . (tep_get_all_get_params(array('oID')) . 'oID=' . $_GET['oID']) . $oscid . '\')">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a>' . $buttons_bottom; ?></td>
          <?php
          ...
          <tr>
            ...
            <td align="right">
              <table border="0" width="100%" cellspacing="0" cellpadding="0">
                <tr><?php 
                    echo tep_draw_form('orders', FILENAME_ORDERS, '', 'get', '', 'SSL').(isset($_GET['osCAdminID']) ? tep_draw_hidden_field('osCAdminID',$_GET['osCAdminID']):''); 
                    tep_hide_session_id();
                  ?>
                ...
                <tr>
                  <?php echo tep_draw_form('status', FILENAME_ORDERS, '', 'get', '', 'SSL').(isset($_GET['osCAdminID']) ? tep_draw_hidden_field('osCAdminID',$_GET['osCAdminID']):''); ?>
                  <td class="smallText" align="right">
                    <?php echo HEADING_TITLE_STATUS . ' ' .$_GET['status']. tep_draw_pull_down_menu('status', array_merge(array(array('id' => '', 'text' => TEXT_ALL_ORDERS)), $orders_statuses), $_GET['status'], 'onChange="this.form.submit();"');
                  tep_hide_session_id(); ?>
                  </td>
                  </form>
                </tr>
...
?>  
                  <img src="images/icons/comment.png" onmouseover="showhint('<?php echo '' . $products . ''; ?>', this, event, '300px'); return false" align="top" border="0">
                </td>
    ...
    switch ($action) {
      ...
      default:
        if (isset($oInfo) && is_object($oInfo)) {
          $heading[] = array('text' => '[' . $oInfo->orders_id . '] ' . $oInfo->customers_name);
         ...
         // RCO start
          if ($cre_RCO->get('orders', 'sidebarbuttons') !== true) {  
            ...
            $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ORDERS_PACKINGSLIP, 'oID=' . $oInfo->orders_id, 'SSL') . '" TARGET="_blank">' . tep_image_button('button_packingslip.gif', IMAGE_ORDERS_PACKINGSLIP) . '</a> <br> <a href="' . tep_href_link(FILENAME_ORDERS_USPS_SHIP, 'oID=' . $oInfo->orders_id) . '" TARGET="_blank">' . tep_image('includes/languages/english/images/buttons/button_labelshipping.gif', IMAGE_ORDERS_USPS_SHIP) . '</a>');
          }
          // RCO eof        
          //RCI start
          ...
          //RCI eof
          $contents[] = array('text' => TEXT_DATE_ORDER_CREATED . ' <b>' . tep_date_short($oInfo->date_purchased) . '</b>');  
          ...
          $contents[] = array('text' => TEXT_INFO_PAYMENT_METHOD . ' <b>'  . $oInfo->payment_method . '</b>');
          //begin PayPal_Shopping_Cart_IPN V3.15 DMG
          if (strstr(strtolower($oInfo->payment_method), 'paypal')) {
        ...
        $contents[] = array('text' => '<br>' .  (preg_match('/Priority/',$ship_method)? $ship_method : '<b>' . $ship_method . '</b>'));

        if (preg_match('/^USPS/',$ship_method)) {

        $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_ORDERS_USPS_SHIP, 'oID=' . $oInfo->orders_id) . '" TARGET="_blank">' .  tep_image('includes/languages/english/images/buttons/button_labelshipping.gif', IMAGE_ORDERS_USPS_SHIP) . '</a>');
