<?php
/*
  $Id: paypalwpp_applicationtop_bottom.php,v 1.0.0.0 2007/11/13 13:41:11 datazen Exp $
...
*/
  ...
  define('FILENAME_PAYPAL_WPP_IPN', 'paypal_wpp_ipn.php');
  ...
  function tep_paypal_wpp_enabled() {    
    ...
    if ( defined('MODULE_PAYMENT_PAYPAL_WPP_STATUS') && MODULE_PAYMENT_PAYPAL_WPP_STATUS == 'True' ) {
  ...
  function create_temp_customer($customer_info) {
    ...
    if (tep_db_num_rows($query) > 0) {
      ...
      $_SESSION['paypalwpp_create_account'] = '1';
    } 
  ...
  if ( defined('MODULE_PAYMENT_PAYPAL_WPP_STATUS') && MODULE_PAYMENT_PAYPAL_WPP_STATUS == 'True' && (strstr($_SERVER['PHP_SELF'], FILENAME_CHECKOUT_SHIPPING) || strstr($PHP_SELF, FILENAME_CHECKOUT_SHIPPING)) && tep_not_null($_GET['token']) && tep_not_null($_GET['PayerID']) ) {    
    require_once(DIR_WS_MODULES . 'payment/paypal_wpp/paypal_wpp_base.php');
    $paypal_wpp = new paypal_wpp_base();
    $response = $paypal_wpp->GetExpressCheckoutDetailsRequest($_GET['token'], $_GET['PayerID']);
  ...
  if ( defined('MODULE_PAYMENT_PAYPAL_WPP_STATUS') && MODULE_PAYMENT_PAYPAL_WPP_STATUS == 'True' && (strstr($_SERVER['PHP_SELF'], FILENAME_CHECKOUT_SHIPPING) || strstr($PHP_SELF, FILENAME_CHECKOUT_SHIPPING)) ) {
  ...
  if ( defined('MODULE_PAYMENT_PAYPAL_WPP_STATUS') && MODULE_PAYMENT_PAYPAL_WPP_STATUS == 'True' && (strstr($_SERVER['PHP_SELF'], FILENAME_CHECKOUT_CONFIRMATION) || strstr($PHP_SELF, FILENAME_CHECKOUT_CONFIRMATION)) && isset($_POST['payment']) && $_POST['payment'] == 'paypal_wpp_ec' ) {
