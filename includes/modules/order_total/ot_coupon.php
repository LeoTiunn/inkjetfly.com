...
global  $currencies,$order,$HTTP_POST_VARS;
  if ($HTTP_POST_VARS['gv_redeem_code']) {

// get some info from the coupon table
  $coupon_query=tep_db_query("select coupon_id, coupon_amount, coupon_type, coupon_minimum_order,uses_per_coupon, uses_per_user, restrict_to_products,restrict_to_categories,coupon_sale_exclude from " . TABLE_COUPONS . " where coupon_code='".tep_db_input($HTTP_POST_VARS['gv_redeem_code'])."' and coupon_active='Y'");
...
//**si** 09-11-05 end
        ...
        if (isset($HTTP_POST_VARS['submit_redeem_coupon_x'])) tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_NO_REDEEM_CODE), 'SSL'));
}
