...
class ChargeIt {
    ...
    function pre_confirmation_check() {
		...
		if ( ($result == false) || ($result < 1) ) {
			...
			tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code . '&error=' . urlencode($payment_error_return), 'SSL', true, false));
		}
...
//echo $this->virtual_merchant_url.'  '.$data;exit();
		unset($response);
	...
	function msgResultTests() {
		...
		if ($this->responseAry['ssl_result'] != '0') {
			...
			$specificErrors = "\n" ;
			$specificErrors = "\n" . $this->avsResponseAry[$this->responseAry['ssl_avs_response']];//. MODULE_PAYMENT_CHARGEIT_EMAIL_ERROR1 
			$specificErrors .= "\n" . $this->cvvResponseAry[$this->responseAry['ssl_cvv2_response']];// . MODULE_PAYMENT_CHARGEIT_EMAIL_ERROR2
	...
	*/
	...
	function get_error() {
      ...
      $error = array('title' => MODULE_PAYMENT_CHARGEIT_CC_ERROR,
                     'error' => strip_tags(stripslashes(urldecode($HTTP_GET_VARS['error']))));
		return $error;
	}
